...
class TestExportXPlaneFiles(XPlaneTestCase):
    ...
    def test_export_collection_from_fresh_blend_file(self):
        """
        Test collection-based export functionality using modern architecture.
        This replaces the legacy layer-based export test.
        """
        ...
        # Set up collection for export using modern collection-based architecture
        collection = bpy.data.collections[collection_name]
        collection.xplane.is_exportable_collection = True
        collection.xplane.layer.name = filename
    ...
    def test_export_collection_with_custom_name(self):
        """
        Test collection-based export with custom layer name.
        This replaces the legacy object-based export test.
        """
        filename = inspect.stack()[0].function

        # Clean up any extra objects that might have been created
        # Keep only the first cube
        cubes = [obj for obj in bpy.data.objects if obj.name.startswith('Cube')]
        if len(cubes) > 1:
            for cube in cubes[1:]:  # Remove all but the first cube
